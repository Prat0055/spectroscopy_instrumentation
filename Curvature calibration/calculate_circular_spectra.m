function spect_cur = calculate_circular_spectra(img, roi)
    % Function to calculate the circular spectra from a given image or a specified region of interest (ROI).
    ...
    % Circle parameters (example values, adjust as needed)
    load circle_parameters.mat
    ...
    % Initialize the array for the circular spectrum
    spect_cur = zeros(1, ul_col);  % Circular spectrum

    % Loop through each column (channel) within the ROI
    for channel = xlower:xupper
        sum_cur = 0;

        % Calculate the Y-coordinate based on the circle model
        for row = ylower:yupper
            % Compute the X-coordinate using the circle equation: (x - xc)^2 + (y - yc)^2 = r^2
           xc=channel-rr;
        yc = m * xc + c;

        
            % yc=m.*xc+c;   
            x = xc + sqrt(rr^2 - (row - yc).^2);
